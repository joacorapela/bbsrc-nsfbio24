...
To facilitate the replication of our experimental setup by other groups,
we will share instructions for building foraging arenas, as well as
...
NaLoDuCo experiments generate extremely large datasets, often exceeding hundreds of terabytes, which makes traditional data distribution methods impractical. Instead, users will need to interact with the data where it is stored. Cloud technologies have matured significantly, offering efficient, scalable solutions for this.

We will store data in the Distributed Archives for Neuroscience1 Data Integration (DANDI), which uses Amazon S3 buckets, and we will provide software to visualise and analyse data in Amazon EC2 instances, to avoid costly data transfers.

This will be a challenging task, since we are the first group openly sharing continuous behavioral and neural recordings on the order of hundreds of terabytes. 

Cloud-based processing of very large volumes of data is a key concern. To address this, we will study the performance of methods addressing the tasks in Figure~\ref{fig:dataAnalysis}. If current methods introduce unacceptable delays, we will explore advanced solutions, including parallel processing techniques and optimizing cloud resource usage.
